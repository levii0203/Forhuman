...
import { setCode, setPreviewIs } from '../../store/slice/preview';



export default function CodeEditor({lang,content,lineCount}){
    const load = useRef(false);
    const dispatch = useDispatch()
    const dark = useSelector((state)=>state.dark.isDark)
    const [collapse,setCollapse] = useState(false);
    const [copy,setCopy] = useState(false);
    const collapsedMsg = lineCount+" hidden lines";
    ...
    return (
        ...
        {collapse ? (
            dark?(
                ...
                <SyntaxHighlighter language={lang} style={tomorrow} className='w-full m-0 overflow-x-auto flex break-words' customStyle={{ paddingTop: 0, marginTop: 0,fontStyle:'italic'}}>
                {`${collapsedMsg}`}
            
                </SyntaxHighlighter>
            ):(
                <SyntaxHighlighter language={lang} style={oneLight} className='w-full m-0 overflow-x-auto flex break-words' customStyle={{ paddingTop: 0, marginTop: 0,fontStyle:'italic'}}>
                {`${collapsedMsg}`}
            ...
            dark?(
                <SyntaxHighlighter language={lang} style={tomorrow} showLineNumbers={true}  className='w-full m-0 overflow-x-auto flex break-words' customStyle={{ paddingTop: 0, marginTop: 0, whiteSpace: "pre-wrap",
                    wordBreak: "break-all",maxWidth: "100%", }}>
                {`${content}`}
                </SyntaxHighlighter>
            ):(
                <SyntaxHighlighter language={lang} showLineNumbers={true} style={oneLight}   className='w-full m-0 overflow-x-auto flex break-words' customStyle={{ paddingTop: 0, marginTop: 0, whiteSpace: "pre-wrap",
                    wordBreak: "break-all",maxWidth: "100%", }}>
                {`${content}`}
                </SyntaxHighlighter>
            )
